...
상단 브랜드 텍스트 **"IT소스 관리포털 — IT Source Management Portal"** 뒤에 UI 버전(`ver1.2.09` 형태)을 표시한다.
...
UI 버전 문자열은 `ver<major>.<minor>.<patch>` 형식을 사용한다.

- **표기 형식**: `major` / `minor` 는 자릿수 그대로, `patch` 는 **2자리 zero-pad** 가 기본 (예: `ver1.2.09`). patch 가 100 이상이 되면 자연스럽게 3자리(`ver1.2.100`)로 확장한다 — 항상 같은 자릿수로 강제 패딩하지 않는다.
- **patch 증가**: 버그 수정, 레이아웃/표 컬럼 재배치, 문구 수정, 로깅/표시 개선 등 **기능 동작은 동일하지만 결과/표시가 개선되는 변경**
- **minor 증가**: 화면/데이터 흐름에 새 기능이 추가되거나 사용자가 체감하는 동작이 확장되는 변경 (예: 새 페이지/카드/필터/다운로드 추가). minor 가 오르면 patch 는 다시 `01` 부터 시작.
- **major 증가**: 모듈 단위의 큰 개편, 기존 사용 흐름/데이터 구조의 큰 변경, 호환성에 영향이 큰 변경. major 가 오르면 minor/patch 는 모두 초기화.
...
|-----|------|------|
| `/` | `/dashboard/` 로 리다이렉트 | — |
| `/dashboard/` | IT소스 관리포털 통합 대시보드 (URL 분석·상태분포+당일 시스템 배치 요약·URL 호출·암복호화 자리) | 공개 |
...
| 공개 경로 | `GET /api/config/global`, `GET /api/config/repos`, `GET /api/schedule/history/dashboard-daily?days=` (`WebConfig`에서 `/api/schedule/**` 예외 — 대시보드는 `days=1`로 당일·배치별 1행 집계, 기간별 상세는 설정 배치 이력) |
...
- **정책 (v1.2~)**: 스냅샷은 **항상 ‘시점 기준 전체(풀) 스냅샷’** 으로 생성된다.
  - 단일 레포 Extract여도 `api_record` 전체를 복제 → `source_repo` 는 **NULL** 로 저장.
  - 호출자가 넘긴 trigger 레포명은 `label` 에 “Extract <repo> @ <시각>” 형태로만 기록(추적성 보존).
  - **레거시 부분 스냅샷**(과거 데이터에서 `source_repo` 가 채워진 행)은 보존하되, viewer/list/resolve 표준 경로에서는 **사용하지 않음**.
- 생성 트리거: 수동 Extract 완료 시 자동 생성 + `POST /api/snapshots` 수동 생성(파라미터 `repoName` 은 무시되고 항상 전체로 생성됨)
- 정리/삭제:
...
- 조회/resolve:
  - `GET /api/snapshot-view/list?date=YYYY-MM-DD` — 해당 일자 범위의 전체 스냅샷 목록(최신순). `repository`/`repositories` 파라미터는 **무시**.
  - `GET /api/snapshot-view/resolve?date=YYYY-MM-DD` — 그 일자 이전(<=) 가장 최신 **전체 스냅샷 1건**(`source_repo IS NULL OR ''`).
  - `GET /api/snapshot-view/{id}/records|counts` — 스냅샷 내부 행 단위 조회/집계. `repository`/`repositories` 는 row 레벨 필터로 그대로 사용.

### 메타 테이블: `api_record_snapshot`
- `id` (PK), `snapshot_at`, `snapshot_type`, `label`, `created_ip`, `source_repo`(신규 행은 항상 NULL — 레거시 호환 보존), `record_count`
