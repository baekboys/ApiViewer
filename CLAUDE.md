...
# 상태 (status) — 자동 5가지 + 수동 2가지

대시보드 "🚦 차단대상 진행사항" 표는 7개 상태값을 **상위 3-tier(사용/차단/보류)** + **세부 13컬럼** 으로 계층화해 보여준다. 매핑 정의는 workflow.html "4. 상태 판정 기준" 섹션 하단 매핑표 참고.
...
| 최우선 차단대상 | 호출 0건 + 커밋 1년 경과 | 자동. `logWorkExcluded` 플래그로 B-① / B-② 분리 |
| 후순위 차단대상 | 수동 설정 (침해사고 로그, IT담당자 검토건) | 수동 |
| 검토필요대상 | 호출 0건+커밋 1년 미만 / 호출 1~`reviewUpperThreshold`건+커밋 1년 경과 | 자동. `callCount` + `recentLogOnly` 로 보류 ④/⑤/⑥/⑦ 4분할 |
| 사용 | 기본값 | — |

## 보조 플래그 (검토필요대상 / 최우선 차단대상 subset 분류)

| 컬럼 | 의미 | true 조건 |
|------|------|----------|
| `log_work_excluded` | 최우선 차단대상 → B-② 분류 | 호출0건 + 로그작업 커밋 제외 시에만 1년 경과 |
| `recent_log_only` | 검토필요대상 + 호출0건 → 보류 ④ 분류 | 1년 미만 커밋이 모두 "로그"/"불필요" 키워드 메시지 |

## 임계값 설정 (GlobalConfig)

| 키 | 기본값 | 의미 |
|----|--------|------|
| `reviewThreshold` | 3 | 보류 ⑥ "호출 1~N건" 의 N. 동시에 ⑥/⑦ 분기 기준 |
| `reviewUpperThreshold` | 10 | 검토필요대상 호출 상한. 이 값 초과면 자동 "사용" |

## 수동 판단 상태 (담당자 결정, 자동 재계산 제외)

`ApiStorageService.MANUAL_STATUSES` 로 정의된 2개 상태값. 사용자가 일괄변경/단건수정으로 이 상태를 선택하면 `statusOverridden=true` 가 자동 ON 되어 추출 재계산 시 보존된다. `calculateStatus` 도 현재 상태가 MANUAL_STATUSES 인 레코드는 그대로 반환(이중 보호).

| 상태 | 의미 | 입력 경로 |
|------|------|----------|
| 차단대상 → 사용 | 자동 판정으론 최우선/후순위 차단대상이지만 담당자 검토(현업 협조전·메일·내부 판단 포함) 결과 사용중으로 확정 | 일괄변경 / 단건수정 / 엑셀 업로드 |
| 검토필요 → 사용 | 자동 판정으론 검토필요대상이지만 담당자 검토 결과 사용중으로 확정 | 동일 |
